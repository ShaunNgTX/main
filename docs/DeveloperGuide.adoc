...
command such as `add` were to be used in Quiz Mode, the ParserManager would deem the command as invalid. +
...
* stores a `RevisionTool` object that represents the `RevisionTool`.
* stores the `Revision Tool` data.
...
Step 3. The AddCommand object will call its `addMcq()` method, this method will read the command and store the `Answerable` with the answers into the `question bank` accordingly.
